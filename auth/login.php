...
<div class="w-full max-w-md bg-surface-container-lowest rounded-xl border border-outline-variant/30 shadow-lg p-8">
    <div class="text-center mb-6">
        ...
        <p class="text-sm text-on-surface-variant">Masuk ke akun <?= htmlspecialchars(APP_NAME, ENT_QUOTES, 'UTF-8'); ?></p>
    </div>
    ...
    <form method="POST" action="" class="space-y-4">
        ...
        <div>
            ...
            <input id="email" type="email" name="email" value="<?= htmlspecialchars($email, ENT_QUOTES, 'UTF-8'); ?>" class="w-full rounded-lg border border-outline-variant px-3 py-2.5" required>
        </div>
        <div>
            <label for="password" class="block text-sm mb-1">Password</label>
            <input id="password" type="password" name="password" class="w-full rounded-lg border border-outline-variant px-3 py-2.5" required>
        </div>
        <button class="w-full rounded-lg bg-primary text-on-primary py-2.5 font-semibold">Masuk</button>
    </form>

    <p class="text-center text-sm mt-5 text-on-surface-variant">Belum punya akun? <a href="register.php" class="text-primary font-semibold">Daftar sekarang</a></p>
</div>
</body>
</html>
